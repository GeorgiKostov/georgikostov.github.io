...
Basecamp's [Shape Up](https://basecamp.com/shapeup/1.2-chapter-03) calls this an **appetite**: start with a time budget and design the work to fit it. It is the opposite of allowing an idea to silently become a three-month project.
...
1. **Model-generated hypothesis** — a plausible lead.
2. **Public signal** — evidence that someone described the problem.
3. **Observed behavior** — evidence of what a customer actually does, sacrifices, or buys.
...
Then identify the dominant risk. Silicon Valley Product Group separates product risk into [value, usability, feasibility, and business viability](https://www.svpg.com/four-big-risks/). Ask which one could kill the idea now, and choose an experiment that targets it.
...
<figure class="article-figure">
  <img src="/assets/images/revenue-per-token/commitment-ladder.svg" alt="Commitment ladder rising from model approval and compliments through attention, access, trial, payment, repeat use, renewal, and referral. Evidential strength increases with observable customer sacrifice.">
  <figcaption>The useful question is not “Did they like it?” but “What did they commit?”</figcaption>
</figure>
...
OpenAI's account of [agent-first engineering](https://openai.com/index/harness-engineering/) makes the same shift visible at software scale: humans increasingly design environments, specify intent, and create feedback loops in which agents can work reliably. Autonomy becomes useful when the harness is good.
...
For an AI product, add model and prompt version, token use, cost, latency, retries, failure category, user feedback, and regressions. Tools such as [Langfuse](https://langfuse.com/docs/observability/overview) can connect traces to tokens, cost, latency, prompts, and evaluations.
...
1. **What did we learn?** Name the evidence, not the output.
2. **Which assumption became safer or more dangerous?** Update the ledger.
3. **What did a customer commit?** Separate praise from sacrifice.
4. **What should die?** Close weak bets before opening new ones.
5. **What is the single active bet?** Concentrate execution.
6. **What is the next cheapest decisive test?** Give it an appetite and a stop condition.
